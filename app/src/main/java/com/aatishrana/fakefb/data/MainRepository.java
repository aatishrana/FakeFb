...
{
    private String dataState = Const.DEFAULT_DATA;
    private APIService service;
    private SharedPreferences mySharedPref;

    private final String defaultData;

    public MainRepository(APIService service, SharedPreferences mySharedPref, String defaultData)
    {
    ...
    public Observable<Boolean> createNewData(final String name, final String url)
    {
        return Observable.create(new ObservableOnSubscribe<Boolean>()
        {
            ...
            {
                if (mySharedPref.contains(name) || name == Const.DEFAULT_DATA)
                {
                    e.onError(new RuntimeException("Data for " + name + " already exist, please use some other name or delete previous data"));
                }

                Response<String> response = service.fetchData(url).execute();
                if (response.isSuccessful())
                {
                    H.E(response.body());
                    FbData fbdata = processData(response.body());
                    if (fbdata != null)
                    {
                        mySharedPref.edit().putString(name.toLowerCase(), response.body()).commit();
                        e.onNext(true);
                        e.onComplete();
                    } else
                        e.onError(new RuntimeException("Incorrect JSON"));
                } else
                    e.onError(new RuntimeException(response.message()));
            }
        });
    }

    public List<com.aatishrana.fakefb.settings.FbData> getListOfAllData()
    {
        dataState = mySharedPref.getString(Const.DEFAULT_SELECTED, Const.DEFAULT_DATA);

        List<com.aatishrana.fakefb.settings.FbData> fbDataList = new ArrayList<>();
        Map<String, ?> allData = mySharedPref.getAll();
        for (Map.Entry<String, ?> data : allData.entrySet())
        {
            if (data.getKey().equalsIgnoreCase(Const.DEFAULT_SELECTED))
                continue;

            boolean isSelected = false;
            if (data.getKey().equalsIgnoreCase(dataState))
                isSelected = true;
            fbDataList.add(new com.aatishrana.fakefb.settings.FbData(data.getKey(), isSelected));
        }
        return fbDataList;
    }

    public boolean removeFbData(String name)
    {
        mySharedPref.edit().remove(name.toLowerCase()).apply();
        dataState = Const.DEFAULT_DATA;
        mySharedPref.edit().putString(Const.DEFAULT_SELECTED, dataState.toLowerCase()).commit();
        return true;
    }

    public Observable<Boolean> loadAFbData(final String name)
    {
        return Observable.create(new ObservableOnSubscribe<Boolean>()
        {
            @Override
            public void subscribe(ObservableEmitter<Boolean> e) throws Exception
            {
                if (mySharedPref.contains(name.toLowerCase()))
                {
                    dataState = name;
                    mySharedPref.edit().putString(Const.DEFAULT_SELECTED, dataState.toLowerCase()).commit();
                    e.onNext(true);
    ...
    public Observable<FbData> getData()
    {
        return Observable.create(new ObservableOnSubscribe<FbData>()
        {
            ...
            {
                String dataString = "";
                if (mySharedPref.contains(dataState))
                    dataString = mySharedPref.getString(dataState, defaultData);

                e.onNext(processData(dataString));
                e.onComplete();
            }
    ...
    FbData processData(String responseString) throws JSONException
    {
    ...
    private List<FeedItem> extractNewsFeed(JSONObject news_feed) throws JSONException
    {
        ...
        System.out.print("aatish " + data.toString());
        return data;
    }
    ...
    private FeedItemPost extractSimplePost(JSONObject data, int rank, String user_pic_url, String user_name, String time, String location, int privacy, int emotions, int comments, int views, int shares) throws JSONException
    {
        ...
        System.out.println("\nNews Feed Post");
        System.out.println("\nNews Feed Post" + feedItem.toString());
        return feedItem;
    }
